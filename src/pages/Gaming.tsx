...
const Gaming = () => {
  ...
  useEffect(() => {
    document.title = `${t.pages.gaming} - ${t.header.title}`;
    window.scrollTo(0, 0);
  }, [t]);
        ...
        >
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-border/50">
            <div className="space-y-2">
              ...
              <p className="text-xl font-medium text-gradient-secondary">{t.gaming.subtitle}</p>
            </div>
          </div>
        </motion.div>

        <section id="gaming-platforms-section">
          <CopyDataSection />
        </section>
      </div>
    </Layout>
